...
export default function RandomSelectionGame() {
  const { raffle, participants, loading, error, canExecute, refresh } = useGameRaffle("RANDOM_PICKER");
  ...
  if (error || !raffle) return <GameEmpty title="No hay sorteo activo para selección aleatoria" detail={error?.message} />;

  return (
    <>
      <WinnerOverlay winner={random.winner} onClose={() => random.setWinner(null)} />
      <GameLayout>
        <GameHeader badge="SISTEMA ACTIVO" badgeColor="bg-blue-100 text-blue-700" title="Selección aleatoria" description="La animación recorre nombres y muestra el resultado final registrado por el sistema." />
        <Context raffle={raffle} execution={random.execution} error={random.error} />
        <div className="grid gap-8 lg:grid-cols-[1.4fr_0.8fr]">
          <div className="space-y-8">
            <RandomAnimation animating={random.animating} currentName={random.currentName} />
            {!canExecute && <Warning text="Este sorteo no está listo para ejecución." />}
            <RandomControls animating={random.animating} startSelection={random.startSelection} duration={random.duration} setDuration={random.setDuration} disabled={!canExecute} />
